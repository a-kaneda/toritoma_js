 ...
 * 敵キャラパラメータ定義。
 */
const ENEMY_DEF: { [key: string]: EnemyParam} = {
    // トンボ
    dragonfly: {
        size: 16,
        width: 16,
...
class EnemyFactory {
    ...
    public static create(x: number, y: number, type: string, scene: PlayingScene): Enemy | null {
        ...
        case 'stagbeetle':
            ...
            break;
        case 'cockroach':
            enemy = new Cockroach(x, y, ENEMY_DEF[type], scene);
            break;
