...
enum STATE {
    INIT = 0,               // 初期状態
    FIVE_WAY_SHOT,          // 5-way弾発射
    ALL_DIRECTION_SHOT,     // 全方位弾発射
    ALL_RANGE_SHOT,         // 地面からの画面全体の弾発射
    STATE_COUNT,            // 状態の種類の数
};
// 状態遷移間隔
...
class Hoenycomb extends Enemy {
    ...
    /** ハチを呼び出す位置 */
    private _hornetPosition: number;
            ...
            case STATE.ALL_RANGE_SHOT:      // 地面からの画面全体の弾発射
                ...
                if (this._shotInterval[0] > ALL_RANGE_INTERVAL) {
                    ...
                    for (let i = 0; i < ALL_RANGE_COUNT; i++) {
                        ...
                        EnemyShot.fireNWay({x: x * (i + 1), y: ALL_RANGE_Y_POSITION}, 
                            Math.PI / 2, 
                            1, 
                            0, 
                            ALL_RANGE_SPEED, 
