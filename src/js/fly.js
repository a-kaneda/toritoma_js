        ...
        switch (this._shotState) {
            ...
            case SHOT_STATE.NWAY:// n-way弾発射
                ...
                if (this._shotInterval[0] > NWAY_SHOT_INTERVAL) {
                    EnemyShot.fireNWay(this._hitArea, Util.calcAngle(this._hitArea, scene.playerPosition), NWAY_SHOT_COUNT, NWAY_SHOT_ANGLE, NWAY_SHOT_SPEED, false, scene);
                ...
                if (this._shotInterval[1] > ARROW_SHOT_WAIT) {
                    ...
                    if (this._shotInterval[2] > ARROW_SHOT_INTERVAL) {
                        ...
                        for (let i = 0; i < ARROW_SHOT_COUNT; i++) {
                            EnemyShot.fireNWay(this._hitArea, Util.calcAngle(this._hitArea, scene.playerPosition), 5, Math.PI / 32.0, ARROW_SHOT_SPEED[i], false, scene);
                        }
            ...
            case SHOT_STATE.ALL_DIRECTION:// 全方位弾発射
                ...
                if (this._shotInterval[1] > N3WAY_INTERVAL) {
                    // 自機へ向けて弾を発射する。
                    EnemyShot.fireNWay(this._hitArea, Util.calcAngle(this._hitArea, scene.playerPosition), 3, Math.PI / 16.0, N3WAY_SHOT_SPEED, false, scene);
                    this._shotInterval[1] = 0;
                }
                break;
            case SHOT_STATE.ALL_RANGE:// 全画面弾発射
                ...
                if (this._shotInterval[0] > ALL_DIRECTION_INTERVAL) {
                    ...
                    for (let i = -this._allRangePosition; i < ScreenSize.STAGE_RECT.width; i += ALL_RANGE_POSITION_INTERVAL) {
                        ...
                        EnemyShot.fireNWay({ x: i, y: ScreenSize.STAGE_RECT.height - 18 }, -Math.PI / 2, 1, 0, ALL_RANGE_SPEED, true, scene);
                    }
                    // 右方向から左方向への弾を発射する
                    for (let i = -this._allRangePosition; i < ScreenSize.STAGE_RECT.height; i += ALL_RANGE_POSITION_INTERVAL) {
                        ...
                        EnemyShot.fireNWay({ x: ScreenSize.STAGE_RECT.width, y: i }, Math.PI, 1, 0, ALL_RANGE_SPEED, true, scene);
                    }
                    // 発射位置の座標を移動する。
